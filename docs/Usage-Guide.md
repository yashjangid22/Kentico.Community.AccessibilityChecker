...
| "Automatic Chromium install failed" / scan fails immediately every time | The scan service downloads Chromium automatically the first time it's needed on a machine; this message means that one-time download couldn't complete — usually a network/firewall issue preventing outbound access from the server. |
| First scan on a new machine takes noticeably longer than expected | Normal — Chromium is downloading automatically in the background (a one-time, ~180MB download). Every scan after that is fast. |
